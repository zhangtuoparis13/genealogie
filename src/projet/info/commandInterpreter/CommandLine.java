...
public class CommandLine {
	...
	/**
	 * A simple interface to enforce how the command classes are executed.
	 * 
	 * @author medined
	 */
	...
	/**
	 * Holds the correspondence of command to java class. The key is the command
	 * and the entry is the java class that gets invoked. The java class must
	 * implement the CommandLine.ICommand interface.
	 */
	...
	/**
	 * Displayed when the HELP command is executed. Provides for customization
	 * option.
	 */
	...
	/**
	 * This variable allows runtime detection of input type: from a file (false)
	 * or from a command line (true).
	 */
	private boolean isInteractive = true;

	/**
	 * Makes the correspondence between the command and the class to invoke.
	 * Notice that the command is trimmed and converted to upper-case.
	 * 
	 * @param cmd
	 *            The command token to assign to clazz.
	 * @param clazz
	 *            The java class that implements the command.
	 */
	public void assignClassToCommand(String cmd, String clazz) {
		cmdToClass.put(cmd.trim(), clazz);
	}

	/**
	 * Invoked by the HELP command. It displays a list of active commands. Note
	 * that the list may change if the assignClassToCommand method is called
	...
	/**
	 * Displays the version, basic command list, and optional help message.
	 */
	public void init() {
	...
	/**
	 * Parses the input stream into tokens. Note that quotes are needed when
	 * command-line parameters have spaces.
	 * 
	 * @param st
	...
	/**
	 * This method executes the commands. There are four standard commands
	 * (exit, help, # and rem). Otherwise, the command is looked up in the
	 * cmdToClass hashmap so that its equivalent java class is found. Then the
	 * class is executed using reflection.
	 * 
	 * @param v
	 *            The command line broken into tokens. Including the initial
	 *            command token.
	...
	private boolean processCommands(Vector v) {
		...
		while (eCmd.hasMoreElements()) {
			...
			if (cmd.equalsIgnoreCase("exit")) {
				...
				// System.out.println("Voici la commande demandée -----> "+finalCmd);
				...
				try {
					...
					e.printStackTrace();
					System.out.println("Error Running: [" + classToInvoke
							+ "] with [" + v + "]");
				}
				break;
			}
	 ...
	 * @param v
	 *            The command line parses into tokens.
	...
	public boolean runMethod(String theName, Vector v) {
		...
		try {
			theInstance = (ICommand) theConstructor.newInstance(new Class[0]);
			if (null == theInstance) {
				throw new RuntimeException("theInstance is null.");
			}
		} catch (InvocationTargetException e) {
			e.getCause().printStackTrace();
			throw new RuntimeException("InvocationTargetException for "
					+ theName);
		} catch (java.lang.NoClassDefFoundError e) {
			throw new RuntimeException("NoClassDefFoundError for " + theName);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("Unable to load constructor for "
					+ theName);
		}
		...
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("Unable to find test method for "
					+ theName);
		}
